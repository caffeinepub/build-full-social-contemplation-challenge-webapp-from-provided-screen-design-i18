...
const INVITATION_STORAGE_KEY = 'social_contemplation_invitation_params';

/**
 * Persists invitation parameters (challengeId + code) to sessionStorage.
 * Used to maintain invitation context across authentication flow.
 */
export function persistInvitationParams(params: { challengeId: bigint; code: string }): void {
    try {
        ...
        sessionStorage.setItem(INVITATION_STORAGE_KEY, serialized);
    } catch (error) {
 ...
 * Returns null if no invitation params are stored.
 */
